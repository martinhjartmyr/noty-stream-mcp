import { ApiResponse, NotyConfig } from '../types.js'

export class NotyApiClient {
  private config: NotyConfig
  private baseHeaders: Record<string, string>

  constructor(config: NotyConfig) {
    this.config = config
    this.baseHeaders = {
      'Content-Type': 'application/json',
      'User-Agent': 'Noty-MCP-Server/1.0.0',
    }
  ...
  updateConfig(newConfig: Partial<NotyConfig>): void {
...
export function createClientFromEnv(): NotyApiClient {
  const config: NotyConfig = {
    apiToken: process.env.NOTY_API_TOKEN || '',
    apiUrl: process.env.NOTY_API_URL || 'https://noty.stream/api',
    defaultWorkspaceId: process.env.NOTY_DEFAULT_WORKSPACE_ID,
    debug: process.env.DEBUG === 'true',
  }

  if (!config.apiToken) {
    throw new Error('NOTY_API_TOKEN environment variable is required')
  }

  return new NotyApiClient(config)
}
