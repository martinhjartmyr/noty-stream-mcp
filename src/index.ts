...
class NotyMcpServer {
  ...
  constructor() {
    ...
    this.server = new Server(
      {
        name: 'noty-stream-mcp',
        version: '1.0.0',
        description:
          'MCP Server for Noty Stream API - A comprehensive note-taking system integration',
      },
      {
        capabilities: {
          tools: {},
  ...
  async run() {
    ...
    if (process.env.DEBUG === 'true') {
      console.error(`🚀 Noty Stream MCP Server starting...`)
      ...
      console.error(`🔗 API URL: ${process.env.NOTY_API_URL || 'https://noty.stream/api'}`)
    }
  ...
  private async validateApiKeyInBackground() {
    try {
      ...
      if (!workspacesResponse || !('data' in workspacesResponse) || !Array.isArray(workspacesResponse.data)) {
        console.error('❌ API key validation failed. Please check your NOTY_API_TOKEN environment variable.')
        return
      }
...
process.on('SIGINT', () => {
  if (process.env.DEBUG === 'true') {
    console.error('🛑 Shutting down Noty Stream MCP Server...')
  }
  process.exit(0)
})
...
// Main execution
async function main() {
  const server = new NotyMcpServer()
  await server.run()
}
